...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pokécoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pokécoin Signed Message:\n";
...
{
    // Pokécoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pokécoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Pokécoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Too tired to write something...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PokécoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PokécoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PokécoinMiner(CWallet *pwallet)
{
    printf("PokécoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pokécoin-miner");
    ...
    try { loop {
        ...
        printf("Running PokécoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PokécoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PokécoinMiner, pwallet));
}
