...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("pokécoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("pokécoin://"))
    {
        uri.replace(0, 11, "pokécoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Pokécoin.lnk";
}
...
{
    return GetAutostartDir() / "pokécoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Pokécoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  pokécoin-qt [" + tr("command-line options") + "]                     " + "\n";
